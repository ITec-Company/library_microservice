...
func TestTagHandler_GetAll(t *testing.T) {
	type mockBehavior func(s *mock_service.MockTagService, limit, offset int)
	...
	testTable := []struct {
	...
	}{
		{
			...
			limit:  0,
			offset: 0,
			mockBehavior: func(s *mock_service.MockTagService, limit, offset int) {
				s.EXPECT().GetAll(limit, offset).Return([]*domain.Tag{domain.TestTag(), domain.TestTag()}, nil)
			},
		...
		},
		{
			...
			offset: 0,
			mockBehavior: func(s *mock_service.MockTagService, limit, offset int) {
				s.EXPECT().GetAll(limit, offset).Return(nil, errors.New("now rows in result"))
			},
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			testCase.mockBehavior(service, testCase.limit, testCase.offset)

			logger := logging.GetLogger("../../../../logs", "test.log")
			TagHandler := http.NewTagHandler(service, logger)
...
func TestTagHandler_GetByUUID(t *testing.T) {
	type mockBehavior func(s *mock_service.MockTagService, uuid string)
	...
	testTable := []struct {
	...
	}{
		{
			...
			input: "1",
			uuid:  "1",
			mockBehavior: func(s *mock_service.MockTagService, uuid string) {
				s.EXPECT().GetByUUID(uuid).Return(domain.TestTag(), nil)
			},
			expectedStatusCode:  200,
...
func TestTagHandler_GetManyByUUIDs(t *testing.T) {
	type mockBehavior func(s *mock_service.MockTagService, uuids []string)
	...
	testTable := []struct {
	...
	}{
		{
			...
			input: "1,2",
			uuids: []string{"1", "2"},
			mockBehavior: func(s *mock_service.MockTagService, uuids []string) {
				s.EXPECT().GetManyByUUIDs(uuids).Return([]*domain.Tag{domain.TestTag(), &domain.Tag{UUID: "2", Name: "Test Tag2"}}, nil)
			},
			expectedStatusCode:  200,
		...
		},
		{
			...
			uuids: []string{"1"},
			mockBehavior: func(s *mock_service.MockTagService, uuids []string) {
				s.EXPECT().GetManyByUUIDs(uuids).Return(nil, errors.New("now rows in result"))
			},
			expectedStatusCode:  404,
	...
	for _, testCase := range testTable {
		t.Run(testCase.name, func(t *testing.T) {
			...
			service := mock_service.NewMockTagService(c)
			testCase.mockBehavior(service, testCase.uuids)
...
func TestTagHandler_Create(t *testing.T) {
	type mockBehavior func(s *mock_service.MockTagService, dto domain.CreateTagDTO)
	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockTagService, dto domain.CreateTagDTO) {
				s.EXPECT().Create(&dto).Return("1", nil)
			},
		...
		},
		{
			...
			dto: *domain.TestTagCreateDTO(),
			mockBehavior: func(s *mock_service.MockTagService, dto domain.CreateTagDTO) {
				s.EXPECT().Create(&dto).Return("", errors.New("some service error"))
			},
			expectedStatusCode:  500,
...
func TestTagHandler_Delete(t *testing.T) {
	...
	testTable := []struct {
		...
		},
		{
			...
			uuid:  "1",
			mockBehavior: func(s *mock_service.MockTagService, uuid string) {
				s.EXPECT().Delete(uuid).Return(errors.New("no rows affected"))
			},
			expectedStatusCode:  404,
...
func TestTagHandler_Update(t *testing.T) {
	type mockBehavior func(s *mock_service.MockTagService, dto *domain.UpdateTagDTO)
	...
	testTable := []struct {
	...
	}{
		{
			...
			mockBehavior: func(s *mock_service.MockTagService, dto *domain.UpdateTagDTO) {
