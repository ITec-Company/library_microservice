	...
	testTable := []struct {
	...
	}{
		{
			...
			mock: func(dto *domain.CreateArticleDTO) {
				...
				mock.ExpectQuery(regexp.QuoteMeta(createArticleQuery)).WithArgs(dto.Title, dto.DirectionUUID, dto.AuthorUUID, dto.Difficulty, dto.EditionDate, 0, dto.Description, dto.URL, dto.Language, pq.Array(dto.TagsUUIDs), 0).WillReturnRows(rows)
				mock.ExpectCommit()
			},
			expectedResult: "1",
			expectError:    false,
		},
		{
			...
			mock: func(dto *domain.CreateArticleDTO) {
				mock.ExpectBegin()
				mock.ExpectQuery(regexp.QuoteMeta(createArticleQuery)).WithArgs(dto.Title, dto.DirectionUUID, dto.AuthorUUID, dto.Difficulty, dto.EditionDate, 0, dto.Description, dto.URL, dto.Language, pq.Array(dto.TagsUUIDs), 0).WillReturnError(errors.New("DB error"))
				mock.ExpectRollback()
			},
			expectedResult: "",
			expectError:    true,
	...
	testTable := []struct {
	...
	}{
		{
			...
			mock: func(dto *domain.UpdateArticleDTO) {
				mock.ExpectBegin()
				result := sqlmock.NewResult(1, 1)
				mock.ExpectExec(regexp.QuoteMeta(updateArticleQuery)).WithArgs(dto.Title, dto.DirectionUUID, dto.AuthorUUID, dto.Difficulty, dto.EditionDate, dto.Rating, dto.Description, dto.URL, dto.Language, pq.Array(dto.TagsUUIDs), dto.DownloadCount, dto.UUID).WillReturnResult(result)
				mock.ExpectCommit()
			},
			expectedError: false,
		},
		{
			...
			mock: func(dto *domain.UpdateArticleDTO) {
				mock.ExpectBegin()
				mock.ExpectExec(regexp.QuoteMeta(updateArticleQuery)).WithArgs(dto.Title, dto.DirectionUUID, dto.AuthorUUID, dto.Difficulty, dto.EditionDate, dto.Rating, dto.Description, dto.URL, dto.Language, pq.Array(dto.TagsUUIDs), dto.DownloadCount, dto.UUID).WillReturnError(errors.New("DB error"))
				mock.ExpectRollback()
			},
			expectedError: true,
		},
