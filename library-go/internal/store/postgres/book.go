...
func (bs *bookStorage) Delete(UUID string) error {
	result, err := bs.db.Exec("DELETE FROM book WHERE uuid = $1", UUID)
	if err != nil {
	...
	if rowsAffected < 1 {
		bs.logger.Errorf("error occurred while deleting book. err: %v.", ErrNoRowsAffected)
